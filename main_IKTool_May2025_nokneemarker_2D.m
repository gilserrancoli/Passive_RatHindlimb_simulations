...
in_folder_nokneemarker=[filepath '\DataMay2025\achillescut_ForwardOnly_nokneemarker\kinematics'];
...
for j=1:length(movS)
    ...
    pos_ankle=table2array(trc_data(2:end,I_ankle:(I_ankle+2)));
    pos_mtp=table2array(trc_data(2:end,I_mtp:(I_mtp+2)));

    for i=1:nf
        [eulerZXY(i,:), pos_knee(i,:), Z_femur(i,:)] = computeHipEulerAnglesFromDistances(pos_hip(i,:), pos_pelvis_bottom(i,:), pos_pelvis_top(i,:), pos_ankle(i,:), lfemur, ltibia);

        knee_angle_deg(i,:) = -computeKneeAngle(pos_hip(i,:), pos_knee(i,:), pos_ankle(i,:));
        
        eulerZXY_ankle(i,:) = computeAnkleEulerAngles(pos_pelvis_bottom(i,:), pos_pelvis_top(i,:), pos_knee(i,:), pos_ankle(i,:), pos_mtp(i,:),Z_femur(i,:));
    end
...
function [eulerZXY, selectedP4, Z_femur] = computeHipEulerAnglesFromDistances(P1, P1a, P1b, P2, l1, l2)
    % Step 1: Define the plane from P1a, P1b, P2
    plane_origin = P1a;
    v1 = P1b - P1a;
    v2 = P2 - P1a;
    ...
    [eulerZXY, ok, Z_femur] = tryEuler(P1, P1a, P1b, P2, selectedP4);

end

function [thetaZXY, valid, Z_femur] = tryEuler(P1, P1a, P1b, P2, P4)
    
    P4=P4(:)';
    valid = true;

    X_pelvis = (P1b - P1) / norm(P1b - P1);
    Z_pelvis = cross(P1a - P1, P1b - P1); Z_pelvis = Z_pelvis / norm(Z_pelvis);
    Y_pelvis = cross(Z_pelvis, X_pelvis); Y_pelvis = Y_pelvis / norm(Y_pelvis);
    R_pelvis = [X_pelvis(:), Y_pelvis(:), Z_pelvis(:)];

    Y_femur = (P1 - P4) / norm(P1 - P4);
    Z_femur = cross(P1b - P1a, P2 - P1a);
    Z_femur = Z_femur / norm(Z_femur);
...
function eulerZXY = computeAnkleEulerAngles(P1a, P1b, P4, P2, P3, Z_femur)
    % Inputs:
    ...
    % Plane defined by P1a, P1b, P2, P4
    v1 = P1b - P1a;
    v2 = P4 - P1a;
