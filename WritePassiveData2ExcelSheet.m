...
writetable(table_passive,xlsxname,'Sheet','passive_properties');
...
writetable(table_zR2,xlsxname,'Sheet','table_zR2');

%%For rat23 - joint moments
...
function [r2all, R2all, R2all_dmean]=ComputeR2values(data)
    ...
    for i=1:size(data.sol_val.out_model,2)
        I=[8,11,12]; %hip knee ankle
        for j=1:3 
            r_aux=corrcoef(data.sol_val.out_model{i}(:,I(j))-mean(data.sol_val.out_model{i}(:,I(j))),data.sol_val.out{i}(:,I(j))-mean(data.sol_val.out{i}(:,I(j))));
            r2all_dmean(i,j)=r_aux(2)^2;
        end
        v1=data.sol_val.out_model{i}(:,[8 11 12])-mean(data.sol_val.out_model{i}(:,[8 11 12]));
        v2=data.sol_val.out{i}(:,[8 11 12])-mean(data.sol_val.out{i}(:,[8 11 12]));
        r_aux=corrcoef(v1(:),v2(:));
        % r_aux=corrcoef(data.sol_val.out_model{i}(:,[8 11 12]),data.sol_val.out{i}(:,[8 11 12]))
    ...
    for i=1:size(data.sol_val.out_model,2)
        ...
        for j=1:3
            ...
            y=zscore(data.sol_val.out{i}(:,I(j)));
            yhat=zscore(data.sol_val.out_model{i}(:,I(j)));
            R2all_dmean(i,j) = 1 - sum((y - yhat).^2) / sum((y - mean(y)).^2);
        end
        v1=zscore(data.sol_val.out_model{i}(:,[8 11 12]));
        v2=zscore(data.sol_val.out{i}(:,[8 11 12]));
        y=v2(:);
        yhat=v1(:);
